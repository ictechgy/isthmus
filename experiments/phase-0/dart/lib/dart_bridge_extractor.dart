...
final class _BridgeFactVisitor extends RecursiveAstVisitor<void> {
  ...
  void _recordChannelVariable(
    ...
    if (arguments.isEmpty || arguments.first is! SimpleStringLiteral) return;
    channelByVariable[declaration.name.lexeme] =
  ...
  void _recordChannelCreation(InstanceCreationExpression node) {
    ...
    if (arguments.isEmpty || arguments.first is! SimpleStringLiteral) return;
    final channel = (arguments.first as SimpleStringLiteral).value;
    facts.add(_channelCreationFact(node, channel));
  }

  /// 미해석 생성자의 리터럴 채널 이름을 기록한다.
  void _recordUnresolvedChannelCreation(MethodInvocation node) {
    final arguments = node.argumentList.arguments;
    if (arguments.isEmpty || arguments.first is! SimpleStringLiteral) return;
    final channel = (arguments.first as SimpleStringLiteral).value;
    facts.add(_fact(node.offset, 'channel-create', channel));
  }
  ...
  void _recordMethodInvocation(MethodInvocation node) {
    ...
    final method = (arguments.first as SimpleStringLiteral).value;
    facts.add(_fact(node.methodName.offset, 'method-invoke', channel, method));
  }
  ...
  BridgeFact _channelCreationFact(
    ...
    return _fact(node.offset, 'channel-create', channel);
  }

  /// 공통 위치 필드를 포함하는 브리지 사실을 만든다.
  BridgeFact _fact(int offset, String kind, String channel, [String? method]) {
