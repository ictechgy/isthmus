...
### GLM packet-review 처분 (반영 3건·확인 3건·기각 4건)

- **반영**: diff 보류 메시지에도 양쪽 스냅샷의 관찰량 숫자를 추가해 관찰량 노출의
  마지막 빠진 경로를 닫았다. `isJsonParseFailure`를 command-support에서 재사용해
  중복을 제거했다. 파서 경계 테스트를 보강했다(등록 플래그를 값으로 사용 거부,
  `--format -- x` 거부, `--` 뒤 단독 `-`, diff 보류 숫자 단언).
- **확인**: 계약 오류 `reason`은 parse·baseline 검증기의 `fail()` 고정 문자열과
  숫자뿐이라 경로·값 노출이 없다. `qualifiedName`의 `%`·`#` 이스케이프는 이번
  변경이 아니라 기존 동작이라 "값이 달라지는 것은 `:` 포함 이름뿐"이라는 호환성
  서술이 정확하다(이스케이프 집합은 README에 문서화). query는 애초에 플래그가
  없는 명령이므로 `-` 시작 질의어는 이제 `--`를 요구한다 — 기존에도 경로는
  거부했던 관례의 확장이다.
- **기각**: (a) diff의 공유 파서 전환 — `--before`/`--after`는 값 플래그가 아니라
  그룹 경계라 파서 모델이 다르고 형식 자체가 위치 기반 계약이다. (b) usage 한 줄에
  새 규약 전부 표기 — README 절이 근거고 한 줄 사용법은 대표 형태만 보인다.
  (c) usage 오류의 원인별 세분화 — 인수 내용 재생을 피하는 현재 설계(정적 사용법만
  출력)를 유지한다. (d) EPIPE 시 `exitCode 0` — 파이프 소비자가 먼저 닫힌 정상
